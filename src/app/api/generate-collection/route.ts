...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[generate-collection] Calling Claude for brief:', briefId, `(${brief.collection_format ?? 'a_la_carte'})`)
    const t0 = Date.now()
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{ role: 'user', content: buildCollectionMessage(brief) }],
    })

    console.log(`[generate-collection] Claude responded in ${Date.now() - t0}ms, stop_reason: ${message.stop_reason}`)
